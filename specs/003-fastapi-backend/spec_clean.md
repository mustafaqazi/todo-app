...
- Better Auth is integrated on the frontend (Next.js) only, NOT on the backend
...
- **Auth Endpoint**: Frontend-driven. Better Auth (Next.js) handles signup/login UI. Frontend calls backend endpoints with credentials and receives JWT token.
- **JWT Generation**: Frontend's Better Auth library generates JWT token with `sub` claim containing user identifier.
- **JWT Validation**: Backend validates JWT signature using BETTER_AUTH_SECRET (HS256) before processing API requests.
- **Token Claim**: JWT payload contains standard `sub` claim, which backend extracts to enforce user isolation.
- **Token Expiration**: Token validity and expiration handled by Better Auth; backend returns 401 on expired/invalid tokens.
- **User Records**: Better Auth manages user storage on frontend; backend uses extracted `sub` for task isolation only.
- **No Backend Auth Endpoints**: Backend does NOT implement /api/auth/*. Frontend handles all authentication UI and token generation.
